        ...
        >
          <v-card :to="`/videos/${video.id}`">
            <v-img
              ...
              height="200px"
            >
              <v-card-title v-html="video.videoName"></v-card-title>
            </v-img>
            <v-card-actions>
              <v-btn disabled icon>
...
export default {
  ...
  methods: {
    getSearchKeywords () {
      this.searchContent = this.$route.query.keywords
      document.title = this.searchContent + document.title
    },
    sendContent () {
      if (this.$route.query.keywords !== this.searchContent) {
        this.$router.push({
          path: '/search',
          query: {
...
.home-banner {
  ...
  background: url('http://www.static.hdiata.com/tag.jpg') center center / cover
    no-repeat;
  text-align: center;
  color: #fff !important;
