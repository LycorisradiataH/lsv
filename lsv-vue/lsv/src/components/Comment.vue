    ...
    <div class="comment-input-wrapper">
      <div style="display: flex">
        <v-avatar size="40">
          ...
          <img
            v-else
            :src="this.$store.state.videoInfo.websiteConfig.touristAvatar"
          />
        </v-avatar>
      ...
      >
        ...
        <div class="comment-meta">
          ...
          <div class="comment-user">
            ...
            <span class="blogger-tag" v-if="item.userId === 1">SA</span>
            <span class="author-tag" v-if="item.userId === authorId">
              作者
            </span>
          </div>
          <!-- 信息 -->
          <div class="comment-info">
            ...
            <!-- 发表时间 -->
            <span style="margin-right: 10px">{{ item.gmtCreate | date }}</span>
            <!-- 点赞 -->
            <span class="like" @click="like(item)">
              <v-icon :color="isLike(item.id)" dense
          ...
          >
            ...
            <div class="reply-meta">
              ...
              <div class="comment-user">
                ...
                <span class="blogger-tag" v-if="reply.userId === 1">SA</span>
                <span class="author-tag" v-if="reply.userId === authorId">
                  作者
                </span>
              </div>
              <!-- 信息 -->
              <div class="comment-info">
                ...
                <span style="margin-right: 10px">
                  {{ reply.gmtCreate | date }}
                </span>
                <!-- 点赞 -->
                <span class="like" @click="like(reply)">
                  <v-icon :color="isLike(reply.id)" dense>
    ...
    listComments () {
      ...
      this.axios
        ...
        .then(({ data }) => {
          ...
          this.current++
          this.count = data.data.count === null ? 0 : data.data.count
    ...
    insertComment () {
      ...
      this.axios.post('/api/comments', comment).then(({ data }) => {
        if (data.status) {
          ...
          this.listComments()
          const isReview =
            this.$store.state.videoInfo.websiteConfig.isCommentReview
          if (isReview) {
  ...
  computed: {
    isLike () {
      return function (commentId) {
        ...
        return commentLikeSet.indexOf(commentId) !== -1 ? 'red' : 'black'
      }
    }
  },
  watch: {
